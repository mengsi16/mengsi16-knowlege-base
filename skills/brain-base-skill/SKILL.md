...
4. 让上层 Agent 只关心 **search / ask / ingest / enrich / feedback / health** 这六类意图
...
| chunk enrichment 补填 | `enrich-chunks` | 是（`chunk-enrichment-agent`） | 检测并补填 chunk 的 title/summary/keywords/questions，自动重新入库 |
...
| 一个 doc_id 的 chunk 缺 enrichment | 补填 title/summary/keywords/questions | `enrich-chunks` |
...
1. 临时把文本写成 `.md`
2. 再走 `upload-agent`
3. 从而继续复用 `knowledge-persistence` 的 chunker.py 生成 chunk，再由 `chunk-enrichment` 补填 enrichment 字段
...
用途：检测并补填指定文档 chunk 的 title/summary/keywords/questions，自动重新入库。
```bash
python bin/brain-base-cli.py enrich-chunks --doc-id <DOC_ID>
```
特点：
1. 扫描 `data/docs/chunks/<doc_id>-*.md`，逐个检测缺失的 enrichment 字段
2. 调用 LLM 生成 title/summary/keywords/3〜5 条合成 QA 问题
3. 写回 chunk frontmatter（**格式硬约束**：闭合 `---` 不可遗漏；keywords/questions 必须用 JSON inline 数组 `["item1", "item2"]`，禁止 YAML 多行列表格式）
4. 删除 Milvus 旧行并重新 ingest

适合：
1. 入库后发现 chunk 缺少 enrichment 字段
2. 重新切分后需要补填 enrichment
3. enrichment 格式错误（如 keywords/questions 用了 YAML 多行列表）需要修复
...
外部强 Agent
  └─ brain-base-cli
       ...
       │         ├─ chunk-enrichment skill
       │         ├─ milvus-cli delete-by-doc-ids + ingest-chunks
       │         └─ frontmatter 格式硬约束（闭合 --- + JSON inline 数组）
       ├─ feedback
