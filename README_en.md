...
| **Self-Evolving Crystallized Layer** (`data/docs/chunks/` + `data/crystallized/`) | `organize-agent` / `knowledge-persistence` | chunks generated by chunker.py + organized solidified answers, shortcut returns for similar questions |
...
sequenceDiagram
    ...
    alt official-doc
        CLS->>KP: chunker.py chunk generation + LLM enrichment
    else community
...
7. Call `knowledge-persistence` to save raw Markdown and generate chunks via `bin/chunker.py`.
8. **For each chunk, call LLM to generate summary / keywords / 3-5 synthetic QA questions**, written to chunk frontmatter.
...
sequenceDiagram
    ...
    KP->>KP: chunker.py chunk generation + LLM enrichment
    KP->>KP: raw + chunks dual persistence
...
2. `chunks` (Crystallized Layer) preserves thematic fragments generated by `bin/chunker.py` and enriched by LLM, grep-able and RAG-ready, suitable for precise retrieval and citation.
3. Milvus is only responsible for storage and retrieval, not for generating embeddings out of thin air.

### Chunk Generation Principles

Physical chunk generation is handled by `bin/chunker.py`; the LLM does not perform character counting or splitting state-machine logic.

`knowledge-persistence` only calls the script to generate base chunks, then asks the LLM to enrich them with summary / keywords / questions.
...
4. `upload-agent`: User local document upload Agent (parallel to `get-info-agent`, the two entry points converge at the `knowledge-persistence` layer). Receives local files → `doc-converter` completes storage (original file archive + raw Markdown persistence + image resource preservation) → then hands off to `knowledge-persistence` for chunk generation, LLM enrichment, and unified ingestion.
...
6. `knowledge-persistence`: calls `bin/chunker.py` to generate chunks, synthetic QA generation, raw persistence (Raw Layer) + chunks persistence (Crystallized Layer), Milvus hybrid persistence. **Common downstream for both get-info and upload entry points.**
...
│   ├── docs/chunks/              # Crystallized Layer — written by knowledge-persistence (chunker.py + LLM enrichment)
...
3. raw/chunks dual-replica persistence + deterministic chunk generation via `bin/chunker.py`.
